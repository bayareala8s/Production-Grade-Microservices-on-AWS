...
4. **AWS Idle Cost Advisor (FinOps)** — Account, Inventory, Cost Analyzer, Recommendation, Notification

See [capstone/README.md](capstone/README.md).

**Implementation (one option at a time):** [capstone/IMPLEMENTATION.md](capstone/IMPLEMENTATION.md)
...
# AWS platform (full course)
./scripts/labs-start.sh --all
./scripts/verify-aws-labs.sh # labs 04-08 on AWS
./scripts/labs-stop.sh       # stop NAT/ALB/ECS — save costs (~$0-2/mo idle)
./scripts/labs-stop.sh --destroy  # full teardown ($0)
./scripts/labs-status.sh     # check running state + cost mode
```
