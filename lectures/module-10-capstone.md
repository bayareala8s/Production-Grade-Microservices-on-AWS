...
1. Select and scope a **capstone domain** (E-Commerce default, Banking, SaaS, or AWS Idle Cost Advisor).
...
| Domain options | 20 min | E-Commerce, Banking, SaaS, Idle Cost Advisor |
...
### 2.4 Option D — AWS Idle Cost Advisor (FinOps)

**Reference:** Course cost control (`labs-start` / `labs-stop`) + [`capstone/templates/option-4-idle-cost-decomposition.md`](../capstone/templates/option-4-idle-cost-decomposition.md).

| Service ideas | Notes |
|---------------|-------|
| Account / Auth | JWT; link AWS account via assume-role |
| Inventory | Discover ECS, ALB, NAT, EIP |
| Cost Analyzer | Idle heuristics + Cost Explorer or rate card |
| Recommendation | Rank findings; suggest stop/destroy (no auto-delete) |
| Notification | `IdleCostFinding` alerts |

**Constraints:** Read-only IAM; recommend-only remediation; 1–2 accounts; keep ≥ 3 services on ECS (not a single boto3 script).

**Focus:** FinOps product thinking — estimate idle \$/month and map to start/stop actions students already practice.

### 2.5 Scoping advice

| Team size | Recommended scope |
|-----------|-------------------|
| Solo | Extend course 4 services + 1 new capability **or** Option 4 with 3–4 services |
| Pair | 4–5 services + saga, payment, or idle-cost analyzer |
