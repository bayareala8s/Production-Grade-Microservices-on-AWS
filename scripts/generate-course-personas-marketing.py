#!/usr/bin/env python3
"""Render BayAreaLa8s Production-Grade Microservices — personas / career-outcomes marketing PNGs."""
...
OUT = ROOT / "docs" / "course-production-grade-microservices-personas.png"
OUT_SQUARE = ROOT / "docs" / "course-production-grade-microservices-personas-square.png"
...
PERSONAS = [
    (
        ...
        "Own APIs end-to-end: FastAPI services, contracts, events, and ECS deploys.",
        "Upskill for senior backend roles — stronger system-design interviews.",
    ),
    (
        "Cloud / DevOps Engineer",
        "Ship VPC, ALB, ECS Fargate, Terraform, and CI/CD with lab cost control.",
        "Stand out for production cloud delivery — not container tutorials.",
    ),
    (
        "Platform / SRE",
        "Observability, IAM least privilege, resilience patterns, and ops runbooks.",
        "Ready for promotion talks on reliability & platform ownership.",
    ),
    (
        "Solutions Architect",
        "C4 + context maps plus 4 capstones (E-commerce · Banking · SaaS · FinOps).",
        "Win trust in client conversations with live AWS microservices proof.",
    ),
    (
        "Career Switcher / Student",
        "Portfolio: multi-service platform, EventBridge flows, and hire-ready demos.",
        "Build evidence employers recognize for cloud-native roles.",
    ),
    (
        "Engineering Manager",
        "Team patterns for service ownership, governance, FinOps, and safe rollouts.",
        "Lead microservices initiatives with standards your org can adopt.",
    ),
]
...
def render_wide() -> Image.Image:
    ...
    draw.text(
        (160, 212),
        "Upskill for cloud-native platform roles — portfolio projects that support\n"
...
def render_square() -> Image.Image:
    ...
    # Square: top 4 personas for readability
    personas = PERSONAS[:4]
    ...
    draw.text((48, 118), "Production-Grade Microservices on AWS", font=subtitle, fill=WHITE)
    draw.rectangle((48, 148, 420, 154), fill=GOLD)
    draw.text(
        (48, 170),
        "Upskill for cloud-native platform roles — portfolio projects that\n"
